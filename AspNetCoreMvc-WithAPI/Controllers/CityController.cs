...
namespace AspNetCoreMvc_WithAPI.Controllers
{
	public class CityController : Controller
	{
		...
		public async Task<IActionResult> Index()
		{
			...
			var result = await http.GetAsync("https://localhost:7136/api/Cities");
		...
		public async Task<IActionResult> Create(CityViewModel model)
		{
			...
			var result = await http.PostAsync("https://localhost:7136/api/Cities", content);
			if(result.IsSuccessStatusCode)
			{
		...
		public async Task<IActionResult> Edit(int id)
		{
			...
			var result = await http.GetAsync($"https://localhost:7136/api/Cities/{id}");
			if (result.IsSuccessStatusCode)
			{
				var jsonData = await result.Content.ReadAsStringAsync();
		...
		public async Task<IActionResult> Edit(CityViewModel model)
		{
			...
			var content = new StringContent(jsonData, encoding: Encoding.UTF8, "application/json");
			var result = await http.PutAsync("https://localhost:7136/api/Cities", content);
			if (result.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
		...
		{
			var http = _httpClientFactory.CreateClient();
			var result = await http.DeleteAsync($"https://localhost:7136/api/Cities/{id}");
			if (result.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
